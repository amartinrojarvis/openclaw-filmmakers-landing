...
const initialForm = {
  name: '', activity: '', website: '', experience: 'principiante', priority: '', tools: '', availability: '', context: '', privacyAccepted: false, earlyStartAccepted: false,
};
...
export function SessionIntakeForm({ sessionId, email }: Props) {
  ...
  return (
    <form onSubmit={submit} className="border border-[#f2eee5]/20 p-5 sm:p-9">
      ...
      <label className="mt-7 flex cursor-pointer items-start gap-3 text-sm leading-6 text-[#f2eee5]/55"><input type="checkbox" checked={form.privacyAccepted} onChange={(e) => update('privacyAccepted', e.target.checked)} required className="mt-1 h-4 w-4 accent-[#ff5a2a]" /><span>Acepto que estos datos se utilicen para preparar y coordinar la sesión conforme a la <Link href="/privacidad" target="_blank" className="text-[#ff5a2a] underline">política de privacidad</Link>. *</span></label>
      <label className="mt-4 flex cursor-pointer items-start gap-3 text-sm leading-6 text-[#f2eee5]/55"><input type="checkbox" checked={form.earlyStartAccepted} onChange={(e) => update('earlyStartAccepted', e.target.checked)} required className="mt-1 h-4 w-4 accent-[#ff5a2a]" /><span>Si elijo una fecha dentro de los próximos 14 días, solicito expresamente que el servicio comience durante el plazo de desistimiento y entiendo que este derecho termina cuando la sesión se haya prestado completamente. *</span></label>
